...
router.post('/taxi/login', function(req,res,next){
  ...
  db.query(queryStr, (err,rows,fields)=>{
    if(!err){
      ...
      let code = len==0 ? 1 : 0
      let message = len ==0 ? "아이디 또는 비밀번호가 잘못입력되었습니다." : "로그인 성공"
...
router.post('/taxi/call', function(req,res){
  ...
  let queryStr = `INSERT INTO tb_call VALUES(NULL, 
  "${userId}","${startLat}","${startLng}",
  "${startAddr}","${endLat}","${endLng}","${endAddr}","REQ","",CURRENT_TIMESTAMP)`
...
router.post('/driver/list', function(req, res){
  ...
  db.query(queryStr, function(err,rows,fields){
    if(!err){
      console.log('driver-lst / rows'+JSON.stringify(rows))
      res.json([{code:0,message:"택시 호출 목록 호출 성공",data:rows}])
    }
    else{
      console.log('driver-lst / err : '+err)
...
router.post('/driver/accept', function(req,res){
  console.log('driver-accpet / req.body '+JSON.stringify(req.body))

  let callId = req.body.callId
  let driverId = req.body.driverId
  let userId = req.body.userId

  console.log('driver-accpet / callId = '+callId+', driverId = ',+driverId)

  if(!(callId && driverId)){
    res.json( [ {code:1 , message: "callId 또는 driverId가 없습니디"}])
    return
  }

  let queryStr = `UPDATE tb_call SET driver_id="${driverId}", call_state="RES" WHERE id=${callId}`
  console.log("driver-accpet / queryStr ", queryStr)
  db.query(queryStr, function(err,rows,fields){
    if(!err){
      ...
      console.log('driver-accpet / err : '+JSON.stringify(err))
      res.json([{code:3,message:"알 수 없는 오류가 발생했습니다", data:err}])
    }
  })
})
