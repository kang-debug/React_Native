...
function Main_Map(): JSX.Element {
    ...
    const callTaxi = async ()=>{
        ...
        apis.call(userId, startLat, startLng, startAddr, endLat,endLng,endAddr)
        .then(response => {
    ...
    const handleLongPress = async (event: any) => {
        ...
        setLoading(true)
        apis.geoCoding(coordinate, query.key)
            .then(reponse => {
                setSelectedAddr(reponse.data.results[0].formatted_address)
                setShownBtn(true)
                setLoading(false)
            })
            .catch(err => {
        ...
        if (selectedAddr) {
            if (title == "출발지") {
                ...
                if (autocomplete1.current) {
                    autocomplete1.current.setAddressText(selectedAddr)
                }
            }
            else {
                setMarker2(selectedLatLng)
                if (autocomplete2.current) {
                    autocomplete2.current.setAddressText(selectedAddr)
                }
            }
            setShownBtn(false)
        }
    ...
    let query = {
        key: "AIzaSyDZZGA5Hb4v46W2Vg1yJMZhlme7DfD9UKo",
        language: "ko",
    ...
    const setMyLocation = () => {
        ...
        Geolocation.getCurrentPosition((position) => {
            ...
            apis.geoCoding(coords, query.key)
                .then(response => {
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            {shownBtn && <View style={{ position: 'absolute', top: hp(50) - 45, left: wp(50) - 75, height: 90, width: 150 }}>
